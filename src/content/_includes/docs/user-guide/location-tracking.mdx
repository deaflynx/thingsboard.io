...
<Aside type="note" title="The app's tracking page is hidden by default">
  Live tracking works without it — a session started from a dashboard action runs and saves either way. To let users open the **Live location tracking** page in the app, enable it in the bundle's <DocLink product={props.product} path="user-guide/mobile-app-center/#layout">Layout</DocLink> step: turn on **Show hidden pages**, then switch the page on. See the <a href="/docs/mobile/pe/live-location-tracking/" target="_blank"><b>Live location tracking</b></a> guide for what the page shows.
</Aside>
...
| Accuracy | Best for |
|---|---|
| **High** | Vehicle or field-worker tracking. Drains the battery fastest |
| **Balanced** *(default)* | Periodic check-ins at moderate battery cost |
| **Low** | Long-running background presence, with minimal battery use |
...
| Field | Limit | Effect |
|---|---|---|
| **Distance** | Limit updates by travelled distance | Skips updates until the device has moved at least this far from the previously reported position |
| **Interval** | Limit updates by time interval | Requests a new position no more often than this. Without it, updates arrive as fast as the device reports them |
| **Duration** | Stop tracking after a maximum duration | Tracking stops by itself once this much time has passed since it started |
